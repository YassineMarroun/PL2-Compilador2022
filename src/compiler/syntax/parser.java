...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Fri Jun 10 10:04:33 CEST 2022
  */
...
class CUP$parser$actions {
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
	...
	cb.addQuadruple("RETURN", exp.getTemporal());

	s.setIntermediateCode(cb.create());
	s.setTemporal(exp.getTemporal());
	...
	if(!(tipoExp instanceof TypeLogico)) {
		semanticErrorManager.semanticFatalError("Error semantico: la expresion en sentencia IF debe ser de tipo logico");
	}
	...
	RESULT = lista;
		...
		int llamadaleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).left;
		int llamadaright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-1)).right;
		LlamadaFuncion llamada = (LlamadaFuncion)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		
	SentenciaLlamadaFuncion s = new SentenciaLlamadaFuncion();

	// CÓDIGO INTERMEDIO
	s.setIntermediateCode(llamada.getIntermediateCode());
	...
	RESULT = s;
		...
		ParametroFuncionOProc parametros = (ParametroFuncionOProc)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
	...
	if(tipo != null) {
		...
		if(tipo instanceof TypeProcedure) {
			...
			IntermediateCodeBuilder cb = new IntermediateCodeBuilder(ambito);
			TemporalFactory tF = new TemporalFactory(ambito);
			ArrayList<Parametro> parametrosFunc = parametros.getParametros();

			// Se recorre el listado de parámetros
			for(Parametro param : parametrosFunc) {
				// Generar el Código Intermedio de cada parámetro
				if(param.isEsExpresion()) {
					cb.addQuadruples(param.getExpresion().getIntermediateCode());
					cb.addQuadruple("PARAM", param.getTemporal());
				} else {
					SymbolIF simbolo = param.getSimbolo();
					if(simbolo instanceof SymbolConstantEntero) {
						TemporalIF temp = tF.create();
						SymbolConstantEntero sEntero = (SymbolConstantEntero)simbolo;
						cb.addQuadruple("MV", temp, new Value(sEntero.getValorEntero()));
						cb.addQuadruple("PARAM", temp);
					} else if(simbolo instanceof SymbolConstantLogico) {
						TemporalIF temp = tF.create();
						SymbolConstantLogico sLogico = (SymbolConstantLogico)simbolo;
						cb.addQuadruple("MV", temp, sLogico.getValorLogico());
						cb.addQuadruple("PARAM", temp);
					} else if(simbolo instanceof SymbolVariable) {
						TemporalIF temp = tF.create();
						Variable var = new Variable((SymbolVariable)simbolo);
						cb.addQuadruple("MVA", temp, var);
						cb.addQuadruple("PARAM", temp);
					}
				}
			}
			Procedure procedimiento = new Procedure(llamada.getNombreProcedure(), ambito, ((TypeProcedure)tipo).getParametros());
			cb.addQuadruple("CALL", procedimiento);
	...
	ParametroFuncionOProc param = new ParametroFuncionOProc(lista.getVariables());
	RESULT = param;
	...
	if(tipo instanceof TypeFunction) {
		...
		TemporalFactory tF = new TemporalFactory(ambito);
		TemporalIF temp = tF.create();
		cb.addQuadruples(llamada.getIntermediateCode());
		cb.addQuadruple("RETFUN", temp);
		exp.setIntermediateCode(cb.create());
		exp.setTemporal(temp);
	...
	RESULT = exp;
		...
		int subprleft = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).left;
		int subprright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-4)).right;
		SeccionSubprograma subpr = (SeccionSubprograma)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-4)).value;
	...
	LabelFactoryIF lF = new LabelFactory();
	LabelIF labelPrincipal = lF.create();
	DefPrincipal defP = new DefPrincipal(labelPrincipal);

	// CÓDIGO INTERMEDIO
	ScopeIF ambito = scopeManager.getCurrentScope();
	IntermediateCodeBuilder cb = new IntermediateCodeBuilder(ambito);
	cb.addQuadruples(subpr.getIntermediateCode());
	cb.addQuadruple("INLABEL", labelPrincipal);
	cb.addQuadruples(sentencias.getIntermediateCode());
	...
	SeccionSubprograma seccion = new SeccionSubprograma();
	RESULT = seccion;
		...
		DefSubprograma def = (DefSubprograma)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	seccion.setIntermediateCode(cb.create());
	...
	DefSubprograma def = new DefSubprograma();
	RESULT = def;
		...
		SelectorPof sub = (SelectorPof)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
		int defSubleft = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).left;
		int defSubright = ((java_cup.runtime.Symbol)CUP$parser$stack.peek()).right;
		DefSubprograma defSub = (DefSubprograma)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	cb.addQuadruples(sub.getIntermediateCode());
	cb.addQuadruples(defSub.getIntermediateCode());
	defSub.setIntermediateCode(cb.create());
	...
	RESULT = defSub;
		...
		SelectorPof func = (SelectorPof)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
	...
	RESULT = func;
		...
		SelectorPof proc = (SelectorPof)((java_cup.runtime.Symbol) CUP$parser$stack.peek()).value;
          ...
          case 40: // funcion ::= DECLAREFUNCION IDEN NT$2 PIZQUIERDO defvariables PDERECHO DELIMFUNC tiporetornofuncion DELIMFUNC seccionconstantes secciontipos seccionvariables seccionsubprograma COMIENZO sentenciaFuncion listaSentenciasFuncion FINBLOQUESENTENCIAS DSENTENCIAS 
            {
              ...
              // propagate RESULT from NT$2
                RESULT = (SelectorPof) ((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-15)).value;
	...
	SelectorPof s = new SelectorPof();
	Procedure procedimiento = new Procedure(nombre, ambito, parametros.getVariables());
	int level = ambito.getLevel();

	IntermediateCodeBuilder cb = new IntermediateCodeBuilder(ambito);
	cb.addQuadruple("INLABEL", procedimiento.getCodeLabel(), new Value(level));
          ...
          case 38: // procedimiento ::= COMIENZOPROCEDIMIENTO IDEN NT$1 PIZQUIERDO defvariables PDERECHO DELIMFUNC seccionconstantes secciontipos seccionvariables seccionsubprograma COMIENZO sentencia listaSentencias FINBLOQUESENTENCIAS DSENTENCIAS 
            {
              ...
              // propagate RESULT from NT$1
                RESULT = (SelectorPof) ((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-13)).value;
	...
	cb.addQuadruples(miembro.getIntermediateCode());
	cb.addQuadruple("MVP", temp, miembro.getTemporal());
	...
	if(simbolosTabla.containsSymbol(nombre)) {
		...
		if(s instanceof SymbolVariable) {
			...
			cb.addQuadruple("MVA", temp1, new Variable((SymbolVariable)s));
			cb.addQuadruple("MVP", temp, temp1);
		} else if(s instanceof SymbolConstantEntero) {
			int posicion = ((SymbolConstantEntero)s).getValorEntero();
			cb.addQuadruple("MV", temp, new Value(posicion));
		}
	...
	cb.addQuadruple("INICIO", new Value(memoria.direccionInicio), defP.getLabelPrincipal());
	...
	// Se muestran los textos del programa
	...
	// Se muestra el código intermedio del programa
