...
/** CUP v0.11a beta 20060608 generated parser.
  * @version Tue Mar 29 16:59:57 CEST 2022
  */
...
class CUP$parser$actions {
  ...
  public final java_cup.runtime.Symbol CUP$parser$do_action(
	...
	// PENDIENTE: solo podemos tener devolver en funcion No en procedimiento
	// PENDIENTE: comprobar que expresion es del mismo tipo que el valor devuelto por la funcion
	...
	//el tipos tiene que ser entero
	if(!(tipoExp1 instanceof TypeEntero)){
		semanticErrorManager.semanticFatalError("Error semántico: tipo de la expresion en sentecia ESCRIBIR no es entero");
	}
	...
	if(simbolosTabla.containsSymbol(nombre)){
		...
		if(!(tipoIndice instanceof TypeEntero)){
			semanticErrorManager.semanticFatalError("Error semántico: indice "+nombre+" en sentencia FOR no es de tipo entero");
		}
		...
		// Los dos tipos tiene que se enteros
		if(!(tipoExp1 instanceof TypeEntero) || !(tipoExp2 instanceof TypeEntero)){
			semanticErrorManager.semanticFatalError("Error semántico: expresion en sentencoa FOR no son enteras");
		}
		...
		semanticErrorManager.semanticFatalError("Error semántico: indice "+nombre+" en sentencia FOR no esta declarado");
	}
	...
	//el tipos tiene que ser logico
	if(!(tipoExp instanceof TypeLogico)){
		semanticErrorManager.semanticFatalError("Error semántico: tipo de expresion en sentencia IF NO es lógicos");
	}
	...
	// Los dos tiene que ser iguales
	if( (tipoIzquierda instanceof TypeEntero &&  tipoParametro instanceof TypeEntero)
	 || (tipoIzquierda instanceof TypeLogico &&  tipoParametro instanceof TypeLogico)  ){
		
		Sentenciadeasignacion sentencia = new Sentenciadeasignacion();
		RESULT = sentencia;
	
	}  else{
		semanticErrorManager.semanticFatalError("Error semántico: tipos incompatibles en la sentencia asignacion");
	}
	...
	if(!simbolosTabla.containsSymbol(identificador)){
		semanticErrorManager.semanticFatalError("Error semántico: identificador "+identificador+" no esta declarado");
	}
	...
	//el tipos tiene que ser logicos
	if(!(tipoExp1 instanceof TypeLogico)){
		semanticErrorManager.semanticFatalError("Error semántico: tipos en operador NO no son lógicos");
	}
	
	// Se genera una expresion de tipo LOGICO
	ScopeIF ambito = scopeManager.getCurrentScope();
	...
	// Los dos tipos tiene que se enteros
	if(!(tipoExp1 instanceof TypeEntero) || !(tipoExp2 instanceof TypeEntero)){
		semanticErrorManager.semanticFatalError("Error semántico: tipos en operador * no son enteros");
	}
	
	// Se genera una expresion de tipo ENTERO
	ScopeIF ambito = scopeManager.getCurrentScope();
	TypeTableIF tiposTabla = ambito.getTypeTable();
	...
	// Los dos tipos tiene que ser logicos
	if(!(tipoExp1 instanceof TypeLogico) || !(tipoExp2 instanceof TypeLogico)){
		semanticErrorManager.semanticFatalError("Error semántico: tipos en operador Y no son lógicos");
	}
	...
	// Los dos tipos tiene que se enteros o los dos logicos
	if( (!((tipoExp1 instanceof TypeEntero) && (tipoExp2 instanceof TypeEntero)))  
	   && (!((tipoExp1 instanceof TypeLogico) && (tipoExp2 instanceof TypeLogico) ))){
		semanticErrorManager.semanticFatalError("Error semántico: tipos en operador == no son enteros");
	}
	...
	// [...]
	// Añadirla funcion en la tabla de tipos
	...
	// Al dectectar el id de la funcion tenemos que abrir ambito
	String nombre = id.getLexema().toUpperCase();
	scopeManager.openScope(nombre);
	...
	// [...]
	// Añadir el procedimiento en la tabla de tipos
	...
	// Al dectectar el id de la procediminento tenemos que abrir ambito
	String nombre = id.getLexema().toUpperCase();
	scopeManager.openScope(nombre);
	...
	if(tiposTabla.containsType(nombreTipo)){
		...
		semanticErrorManager.semanticFatalError("Error semántico: el tipo "+nombreTipo+" no esta declarado");
	}	
	...
	if(valor.isEsValorEntero()){
		...
		// SymbolConstantEntero(ScopeIF scope, String name, TypeIF type, int valorEntero)
